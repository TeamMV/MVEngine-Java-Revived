...
public class Oval extends Shape {
    ...
    Oval(Vector2f centre, float rad) {
        super(MVEngine.instance().getPhysics(), centre);
        x = centre.x;
        y = centre.y;
        a = rad;
        b = rad;
    ...
    public Oval(Vector2f focA, Vector2f focB, float c) {
        super(MVEngine.instance().getPhysics(), (focA.x + focB.x) / 2, (focA.y + focB.y) / 2);
        x = center.x;
        y = center.y;
    ...
    public Oval(float x, float y, float radA, float radB) {
        super(MVEngine.instance().getPhysics(), x, y);
        this.x = x;
        this.y = y;
        a = radA;
        b = radB;
    ...
    @Override
    public boolean equalsType(Shape shape) {
